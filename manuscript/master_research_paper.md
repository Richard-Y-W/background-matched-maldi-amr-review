# Archived Master Research Draft

This Markdown draft records an earlier manuscript state. The current submission-oriented manuscript is `manuscript/main.tex`, with the current figure strategy summarized in `manuscript/master_paper_journal_strategy.md`. Use this file as historical drafting context only; current claims, Sa/Oxa results, model-family matrix, and figure order should be taken from the LaTeX manuscript and source-data files.

# Background-Matched Evaluation Reveals Resistant-Population Background Sensitivity In MALDI-TOF AMR Prediction
...
All analyses operate on a flat prediction table with isolate identifier, site, year, organism, drug, binary AST label, and model probability. The main audit engine is `run_background_audit_framework.py`. Publication outputs are created by `scripts/make_final_framework_tables_figures.py` and related figure scripts. Weis/Borgwardt published-code prediction exports are created by `scripts/export_weis_predictions_for_audit.py` and parity checks are run by `scripts/compare_weis_raw_metrics.py`.
...
B.K. conceived the audit framework, implemented the analyses, created outputs, and wrote the manuscript. Additional author contributions should be completed and approved before submission.
