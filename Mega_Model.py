...
Implemented safeguards and analysis features:
...
10. Core result tables plus occlusion saliency exports
...
def intraclass_mixup(x, y, org_id, alpha=MIXUP_ALPHA):
    """Mix only within samples that share the same label and organism."""
    if alpha <= 0:
        return x, y
    mixed = x.clone()
...
def learn_ensemble_temperature(models, val_samples):
    """Fit scalar T on ensemble-averaged validation logits."""
...
def dataframe_to_markdown(df, columns=None):
    """Small dependency-free markdown renderer for manuscript artifacts."""
    if df.empty:
        return "_No rows._\n"
    if columns is None:
...
def compute_mechanism_confound_checks(metric_rows, active_pairs):
    """Small checks for sample size, prevalence, organism, site, and pair effects."""
    rows = []
    df = pd.DataFrame([r for r in metric_rows if r.get("organism") != "__micro__"])
    if df.empty:
        return rows
...
# ═══════════════════════════════════════════════════════════════════════════════
# Ablation training, calibrated before safety gate
# ═══════════════════════════════════════════════════════════════════════════════
...
def train_all_seeds(train_s, val_s, target_site_data, ckpt_dir, mae_ckpt_path=None,
    ...
    if mae_ckpt_path and Path(mae_ckpt_path).exists():
        ...
        # Exclude same-site test-year spectra from MAE pretraining.
...
def run_experiment(args):
    ...
    # ═════════════════════════════════════════════════════════════════════════
    # Multi-organism CNN evaluation
    # ═════════════════════════════════════════════════════════════════════════
    log.info("\n=== Multi-Organism Evaluation (CNN) ===")
    ...
    if getattr(args, "no_saliency", NOTEBOOK_NO_SALIENCY):
        log.info("\n=== Saliency / Interpretability skipped ===")
    else:
        log.info("\n=== Saliency / Interpretability ===")
    ...
    # ═════════════════════════════════════════════════════════════════════════
    # Leave-one-drug-out drug fingerprint experiments
    # ═════════════════════════════════════════════════════════════════════════
    if args.with_leave_one_drug_out:
        log.info("\n=== Leave-One-Drug-Out Diagnostic ===")
