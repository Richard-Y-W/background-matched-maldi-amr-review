...
def ensure_mega_model_compat(repo_root: Path, output_root: Path) -> Path:
    ...
    compat_path.write_text(compat_text, encoding="utf-8")
...
def build_pipeline_steps(config: PipelineConfig) -> list[PipelineStep]:
    ...
    return [
        ...
        PipelineStep(
            ...
            ecoli_export_dir / "lgbm_multi_predictions_long.csv",
        ),
        PipelineStep(
            "audit_ecoli_lgbm_multi",
            audit_command(
                config,
                predictions_csv=ecoli_export_dir / "lgbm_multi_predictions_long.csv",
                model_name="LGBM-multi-ecoli6",
                output_dir=output_path(config, "ecoli_lgbm_multi_background_audit"),
            ),
            output_path(config, "ecoli_lgbm_multi_background_audit/background_matched_audit_summary.csv"),
        ),
        PipelineStep(
...
def dry_run_fallback_path(label: str, error: FileNotFoundError, fallback: Path) -> Path:
    print(
        f"WARNING: {label} was not discovered; dry-run is using fallback path {fallback}. "
    ...
    *,
    ...
    try:
        ...
        if not allow_missing:
            raise
        data_root = dry_run_fallback_path("DRIAMS data root", exc, DEFAULT_DRIAMS_ROOT)
    try:
        ecoli_run_dir = discover_run_dir(ECOLI_RUN_NAME, roots, explicit=args.ecoli_run_dir)
    except FileNotFoundError as exc:
        if not allow_missing:
            raise
        ecoli_run_dir = dry_run_fallback_path("E. coli run directory", exc, DEFAULT_ECOLI_RUN_DIR)
    try:
        saureus_run_dir = discover_run_dir(SAUREUS_RUN_NAME, roots, explicit=args.saureus_run_dir)
    except FileNotFoundError as exc:
        if not allow_missing:
            raise
        saureus_run_dir = dry_run_fallback_path("S. aureus run directory", exc, DEFAULT_SAUREUS_RUN_DIR)
