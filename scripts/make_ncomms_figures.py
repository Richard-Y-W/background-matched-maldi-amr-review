#!/usr/bin/env python3
"""Compatibility wrapper for the current manuscript figure builder.

The previous ReportLab implementation is kept below for audit history, but the
publication figures are created by scripts/make_figures_mpl.py.
"""
...
def axis_auc(c: canvas.Canvas, x0: float, x1: float, y: float, xmin: float = 0.45, xmax: float = 0.90, chart_h: float = 205) -> None:
    c.setStrokeColor(LIGHT_GRAY)
    c.setLineWidth(0.6)
    for tick in [0.5, 0.6, 0.7, 0.8, 0.9]:
        x = xmap(tick, x0, x1, xmin, xmax)
        c.line(x, y, x, y + chart_h)
        centered_text(c, x, y - 13, f"{tick:.1f}", size=7, fill=GRAY)
    c.setStrokeColor(MID_GRAY)
    c.setLineWidth(1.0)
    c.line(xmap(0.5, x0, x1, xmin, xmax), y, xmap(0.5, x0, x1, xmin, xmax), y + chart_h)
...
def figure_3_model_replication(model_df: pd.DataFrame, matrix_df: pd.DataFrame | None = None) -> Path:
    ...
    for model, raw_col, cen_col, px in panels:
        ...
        y0 = 1.55 * inch
        chart_h = (h - 1.55 * inch) - y0
        axis_auc(c, x0, x1, y0, xmin, xmax, chart_h=chart_h)
...
def _pair_panel(c: canvas.Canvas, pair_label: str, subtitle: str, col,
    ...
    for i, row in enumerate(sub.itertuples(index=False)):
        ...
        text(c, x1 + 0.12 * inch, y - 3,
             f"={delta:+.3f}; ret={row.matched_retention_pct:.1f}%", 7.2, False, GRAY)
...
def main() -> None:
    from make_figures_mpl import main as make_current_figures

    make_current_figures()
    return
