...
def parse_years_arg(text: str | None) -> list[str] | None:
    """Parse a comma-separated year override.

    ``None``, an empty string, and ``auto`` mean "use DRIAMSDatasetExplorer".
    Keep years as strings because maldi-learn accepts either strings or lists
    of strings and this preserves the exact command-line representation.
    """
    if text is None:
        return None
...
def select_train_indices(
    ...
    *,
    ...
    ``stratified`` preserves the original exporter behavior. ``all`` is useful
    for temporal exports where the dataset has already been restricted to
    pre-test years, for example DRIAMS-A 2015-2017 -> A-2018.
    """
    if policy == "all":
        return np.arange(len(meta), dtype=int)
    if policy == "stratified":
        train_idx, _ = split_fn(meta, antibiotic=drug, random_state=seed)
        return to_positional_indices(meta, train_idx, name=name)
    raise ValueError(f"Unknown train row policy: {policy}")
...
def write_reproduction_report(
    ...
    report_json = {
        ...
        **source_metadata,
        ...
        "train_row_policy": args.train_row_policy,
        "model": args.model,
        "seed": args.seed,
        "n_folds": args.n_folds,
        "train_site": args.train_site,
        "train_years": parse_years_arg(args.train_years) or args.train_years,
        "test_sites": split_csv_arg(args.test_sites),
        "test_years": parse_years_arg(args.test_years) or args.test_years,
...
- Train row policy: `{args.train_row_policy}`
...
def run(args: argparse.Namespace) -> None:
    ...
    (output_dir / "RUN_STARTED.txt").write_text(
        ...
        f"train_row_policy={args.train_row_policy}\n"
    ...
    append_progress(
        ...
        f"train_years={args.train_years} test_sites={args.test_sites} "
        f"test_years={args.test_years} panel={args.panel} "
        f"train_row_policy={args.train_row_policy} "
        f"external_row_policy={args.external_row_policy} "
    ...
    train_years = parse_years_arg(args.train_years) or explorer.available_years(args.train_site)
    ...
    for test_site in test_sites:
        ...
        test_years = parse_years_arg(args.test_years) or explorer.available_years(test_site)
        ...
        for species, drugs in species_panels:
            ...
            for drug in drugs:
                ...
                train_idx = select_train_indices(
                    train_meta,
                    train_strat_fn,
                    drug=drug,
                    seed=args.seed,
                    policy=args.train_row_policy,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--train-years",
        default="auto",
        help="Comma-separated train-year override; auto uses all available years for the train site.",
    )
    parser.add_argument(
        "--test-years",
        default="auto",
        help="Comma-separated test-year override; auto uses all available years for each test site.",
    )
    parser.add_argument(
        "--panel",
    ...
    parser.add_argument(
        ...
        default="stratified",
        help=(
            "stratified preserves the legacy Weis training split. all trains on "
            "every eligible loaded training row, useful for temporal A-2018 exports."
        ),
    )
