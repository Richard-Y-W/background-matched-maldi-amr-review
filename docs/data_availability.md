...
This repository is organized so reviewers can inspect the framework code, run the model-agnostic audit on prediction CSVs, and regenerate the included publication tables and figures. It intentionally does not redistribute most raw clinical or sequencing data.
