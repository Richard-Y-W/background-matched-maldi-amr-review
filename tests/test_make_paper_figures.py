...
class MakePaperFiguresTests(unittest.TestCase):
    def test_output_dir_is_forwarded_to_builder(self):
        ...
        builder_command = run.call_args_list[0].args[0]
        self.assertIn("--output-dir", builder_command)
        self.assertIn(str(out_dir), builder_command)

        manuscript_command = run.call_args_list[1].args[0]
        self.assertIn(str(make_paper_figures.MANUSCRIPT_FIGURE_BUILDER), manuscript_command)
