...
class RunModelClassMatrixPipelineTests(unittest.TestCase):
    ...
    def test_dry_run_config_uses_fallback_paths_when_data_and_runs_missing(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = pathlib.Path(tmp)
    ...
    def test_builds_expected_lgbm_export_audit_and_matrix_steps(self):
        ...
        self.assertIn("ecoli_lgbm_multi_background_audit", command_text)
        self.assertIn("lgbm_multi_predictions_long.csv", command_text)
